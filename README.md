...
> **Three of the four variants are validated end to end against real transports;
> one is not.** [`COVERAGE.md`](COVERAGE.md) is the precise statement of what
> this repository does and does not prove, with the evidence and the reason
> for the remaining gap. Read that before relying on this as validation evidence.
...
### Known limitation: `full-matrix` isn't fully green

One leg remains blocked. Full detail, including why it is not closable in
this CI model at all, is in [`COVERAGE.md`](COVERAGE.md).
